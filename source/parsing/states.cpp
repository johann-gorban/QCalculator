...
void ParseStartState::handle_token(ParserContext& context, token_ptr token) {
    ...
    token_ptr extended_token(nullptr);

    TokenType token_type = token->get_type();
    const std::string token_data = token->get_data();

    if (token_type == TokenType::Operator) {
        extended_token = std::make_shared<OperatorToken>(*token, 0, Associativity::Right, Arity::Unary);
        ...
        extended_token = std::make_shared<NumberToken>(*token);
        context.append_extended_token(extended_token);
        ...
        extended_token = std::make_shared<FunctionToken>(*token);
        context.append_extended_token(extended_token);
...
void ParseNumberState::handle_token(ParserContext& context, token_ptr token) {
    ...
    if (token_type == TokenType::Number) {
        ...
        extended_token = std::make_shared<OperatorToken>(*token, 0, Associativity::Left, Arity::Binary);
        context.append_extended_token(extended_token);
        next_state = std::make_shared<ParseBinaryOperatorState>();
    }
    else if (token_type == TokenType::Separator) {
        extended_token = std::make_shared<SeparatorToken>(*token);
        context.append_extended_token(extended_token);
